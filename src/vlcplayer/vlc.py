...
if __name__ == '__main__':
    ...
    if sys.argv[1:] and sys.argv[1] not in ('-h', '--help'):
        ...
        t1=player.get_time();
        t2=player.get_length();
        t=time.strftime('%H:%M:%S',time.gmtime(float(t1/1000)))+'/'+time.strftime('%H:%M:%S',time.gmtime(float(t2/1000)))
        ...
        def sec_forward():
            """Go forward one sec"""
            tt=player.get_time() + 1000
            if(tt>player.get_length()):
                tt=player.get_length;
            player.set_time(tt)
        ...
        def set_start():
            global st,pushflag;
            """Go backward one frame"""
            st=player.get_time()/1000;
            pushflag=0;
            
        def set_end():
            """Go backward one frame"""
            global st,et,times,pushflag,clipindex;
            if pushflag>0:
                times.pop()
                clipindex=clipindex-1;
            et=player.get_time()/1000;
            times.append('-ss %s -t %s -map 0 %s.mkv'%(time.strftime('%H:%M:%S',time.gmtime(float(st))),time.strftime('%H:%M:%S',time.gmtime(float(et-st))),str(clipindex)))
            clipindex=clipindex+1;
            pushflag=1;
            
        def cancel_last():
            """Go backward one frame"""
            global times,pushflag,clipindex;
            times.pop();
            clipindex=clipindex-1;
            pushflag=0;
        ...
        try:
            print('Video size: %s' % str(player.video_get_size(0)))  # num=0
            w=player.video_get_size(0)[0];
            h=player.video_get_size(0)[1];
           #print('Window:' % player.get_hwnd()
        except Exception:
            print('Error: %s' % sys.exc_info()[1])
        while True:
            ...
            print('Timing:%s'%t)
	    cmd='vls \''+string.join(times)+'\' \''+sys.argv[1]+'\' \''+str(w)+'x'+str(h)+'\'';
            print(cmd)
            k = getch()
            print('> %s' % k)
            if k in keybindings:
